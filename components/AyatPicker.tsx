...
export default function AyatPicker({ initialMood, onSelectVerse, onBackToHome }: AyatPickerProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-3.5 sm:px-6 py-4 sm:py-8 md:py-10">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6 pb-4 sm:pb-5 border-b border-zinc-200">
        <div>
          {onBackToHome && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-900 mb-2 transition-colors active:scale-95"
            >
          ...
          <p className="text-xs sm:text-sm text-zinc-600 mt-0.5">
            Pilih ayat yang menyentuh dari tema pilihan atau cari langsung 114 surah.
          </p>
        </div>

        {/* Tab switcher */}
        <div className="flex p-1 rounded-xl bg-zinc-200/70 w-full sm:w-auto shrink-0 self-start sm:self-auto">
          <button
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
            {THEME_CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs whitespace-nowrap transition-all active:scale-95 ${
                  selectedCategory === cat.id
                    ? 'bg-emerald-900 text-white font-medium shadow-2xs'
                    : 'bg-[#F2F0E8] text-zinc-700 hover:bg-zinc-200/80 border border-zinc-200/80'
                }`}
              >
                <span>{cat.label}</span>
              </button>
      ...
      {activeTab === 'curated' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {filteredCuratedVerses.length === 0 ? (
            <div className="col-span-full text-center py-12 bg-white rounded-2xl border border-zinc-200 p-4">
            ...
            filteredCuratedVerses.map((item) => (
              <div
                key={item.id}
                className="bg-white border border-[#E5E2D6] rounded-2xl p-4 sm:p-6 flex flex-col justify-between hover:border-emerald-700 hover:shadow-xs transition-all group relative text-left"
              >
                <div>
                  {/* Top Badge & Audio */}
                  <div className="flex items-center justify-between gap-2 mb-2.5">
                    <div className="flex items-center gap-1.5 truncate">
                      <span className="px-2.5 py-0.5 rounded-full bg-emerald-900/10 text-emerald-900 text-[11px] font-semibold border border-emerald-900/15 shrink-0">
                  ...
                  <p className="text-xs sm:text-sm text-zinc-700 font-serif-elegant italic leading-relaxed mb-2.5">
                    &quot;{item.translation}&quot;
                  </p>

                  {/* Context Note */}
                  <p className="text-[11px] text-zinc-500 bg-[#FAF9F5] p-2.5 rounded-lg border border-zinc-100 mb-3.5">
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-emerald-900 hover:bg-emerald-950 text-white text-xs font-medium transition-all flex items-center justify-center gap-1.5 shadow-2xs active:scale-[0.98]"
                >
                  <Check className="w-3.5 h-3.5" />
                  <span>Pilih Ayat Ini</span>
                </button>
              </div>
            ))
          )}
      ...
      {activeTab === 'browse' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6">
          ...
          <div className="lg:col-span-5 space-y-2 sm:space-y-3">
            ...
            <div className="max-h-[300px] lg:max-h-[500px] overflow-y-auto space-y-1 pr-1">
              {filteredSurahs.map((surah) => (
                <button
                  ...
                  className={`w-full p-2.5 sm:p-3 rounded-xl border text-left transition-all flex items-center justify-between active:scale-[0.99] ${
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span
                      className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-[11px] sm:text-xs font-bold ${
                        selectedSurahNumber === surah.nomor
          ...
          <div className="lg:col-span-7 bg-white rounded-2xl border border-zinc-200 p-4 sm:p-6 flex flex-col justify-between min-h-[380px]">
            <div>
              {/* Surah Title Bar */}
              ...
              {isLoadingSurah ? (
                ...
                <div className="py-3 sm:py-4 space-y-3 sm:space-y-4">
                  ...
                  {currentAyatItem && (
                    <div className="p-3.5 sm:p-5 rounded-2xl bg-[#FCFCF9] border border-[#E2DFD2] space-y-2.5">
                      ...
                      {/* Arabic */}
                      <div className="text-right font-arabic text-xl sm:text-2xl text-emerald-950 font-normal leading-[2.3] select-all" dir="rtl">
            ...
            {currentAyatItem && surahDetail && (
              <button
                ...
                className="w-full mt-3 py-2.5 sm:py-3 rounded-xl bg-emerald-900 hover:bg-emerald-950 text-white text-xs sm:text-sm font-medium transition-all flex items-center justify-center gap-1.5 shadow-2xs active:scale-[0.98]"
              >
                <Check className="w-3.5 h-3.5" />
